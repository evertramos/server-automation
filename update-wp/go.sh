...
if [[ $NEW_URL == "" ]]; then
    ...
    if [[ "$NO_START" != true ]] && [[ "$SERVICE_RUNNING" == true ]]; then
        ...
        if [[ "$USER_ACTION_RESPONSE" != true ]]; then
            echoerror "The site '$DOMAIN' is running. Please make sure to inform the NEW URL for the cloning site or stop the current site services before clonning."
        fi
    else
...
if [[ "$FOLDER_EXIST" == true ]]; then
    echoerror "Your destination folder '$DESTINATION_FOLDER/$DOMAIN' already exists. Please make sure to place your clone site in a new and empty folder and services are not running."
fi
...
if [[ "$CONTAINER_RUNNING" == true ]]; then
    ...
    echoerror "There is a clone site for the '$DOMAIN' site alreary running. Please check the files under the folders: '$SITE_COTNAINER_COMPOSE_WORKDIR_RESULT' and '$DB_COTNAINER_COMPOSE_WORKDIR_RESULT'"
fi

#echowarning "This is just a sample... please consider running your script now!"
#exit 0
