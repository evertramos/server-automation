...
if [[ "$DOCKER_USER_EXISTS_IN_CONTAINER" != true ]]; then
    echoerror "The user '$USER_NAME' does not exist in container the ssh-container: '$SSH_MANAGER'.\
...
# the default user's key from ssh-manager container will be used
#-----------------------------------------------------------------------
if [[ $ARG_KEY_STRING == "" ]]; then

    # Get user's pub key
    run_function docker_get_user_ssh_pub_key $SSH_MANAGER $USER_NAME

    USER_SSH_KEY="$DOCKER_USER_SSH_PUB_KEY"

    if [[ $USER_SSH_KEY == "" ]]; then
        echoerror "We could not find the pub key for the user '$USER_NAME' in container '$SSH_MANAGER'"
    fi
else
...
if [[ ! "$SILENT" == true  ]]; then
    echosuccess "You can access the sites selected using the key from '$USER_NAME': \
      \nssh $USER_NAME@$SSH_MANAGER -t 'ssh $SITE_CONTAINER'"
fi
