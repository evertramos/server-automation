...
# Script to install the Wordpress plugins from the list in a file
...
install_plugins() {
    ...
    do
        if [[ "$DEBUG" == true ]]; then
            echo "${blue}>>> Installing the plugin: $line ${reset}"
        fi
        LOCAL_PLUGIN_NAME=$line

        # Need to change the stdin in order to run other lines ("< /dev/null" at the end)
        docker-compose run --rm wpcli plugin install $LOCAL_PLUGIN_NAME < /dev/null
