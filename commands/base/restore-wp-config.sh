...
# Script to restore the wp-config for the new site
...
restore_wp_config()
{
    ...
    if [[ "$DEBUG" == true ]]; then
        echo "Restore wp-config.php for "$LOCAL_NEW_URL" with debug = "$WP_DEBUG
    fi
    ...
    # it will restore after all files are ready in the clone script
    LOCAL_WP_PATH=$DESTINATION_FOLDER"/"$DOMAIN"/data/site/wordpress-core"

    cd $LOCAL_WP_PATH
    sudo sed -i "/define( 'WP_DEBUG/c\\$LOCAL_WP_DEBUG" wp-config.php
    sudo sed -i "/WP_HOME/d" wp-config.php
    sudo sed -i "/WP_SITEURL/d" wp-config.php
    cd - > /dev/null 2>&1    
}
